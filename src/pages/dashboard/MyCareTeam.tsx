  ...
  return (
    <div className="flex-1 min-w-0 mb-2 lg:mb-0 h-[420px]">
      <Card className="w-full h-full flex flex-col shadow-2xl hover:-translate-y-1 transition-all duration-200 bg-white dark:bg-gradient-to-r from-gray-800 to-zinc-800 border border-gray-200 dark:border-zinc-800">
        ...
        <CardContent className="flex-1 p-2 sm:p-3 lg:p-4 overflow-hidden">
          <ul className="space-y-1.5 sm:space-y-2 lg:space-y-2.5 h-full">
            {team.slice(0, 5).map((member) => (
              <li
                key={member.name}
                className="flex items-center justify-between gap-1.5 sm:gap-2 lg:gap-3 cursor-pointer hover:bg-accent rounded-lg transition px-1.5 sm:px-2 py-1.5 sm:py-2 relative"
                onClick={() => setChatMember(member)}
              >
                <div className="flex items-center gap-1.5 sm:gap-2 lg:gap-3 flex-1 min-w-0">
                  <div className="relative flex-shrink-0">
                    <Avatar className="w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8 xl:w-10 xl:h-10">
                      <AvatarImage src={member.img} alt={member.name} />
                      <AvatarFallback className="text-xs sm:text-sm">{member.name.split(' ').map(n => n[0]).join('')}</AvatarFallback>
                    </Avatar>
                    {member.unread && (
                      <span className="absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 w-1.5 h-1.5 sm:w-2 sm:h-2 lg:w-3 lg:h-3 bg-red-500 border border-white rounded-full"></span>
                    )}
                  </div>
                ...
                <div className="flex gap-0.5 sm:gap-1 lg:gap-1.5 flex-shrink-0">
                  <Button
                    ...
                    size="sm"
                    title="Call"
                    className="hover:bg-green-50 dark:hover:bg-green-900/20 rounded p-0.5 sm:p-1 transition h-5 w-5 sm:h-6 sm:w-6 lg:h-7 lg:w-7 xl:h-8 xl:w-8 text-green-600 dark:text-green-400 hover:text-green-700 dark:hover:text-green-300"
                    onClick={e => { e.stopPropagation(); alert(`Calling ${member.name}...`); }}
                  >
                    <Phone className="w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-3.5 lg:h-3.5 xl:w-4 xl:h-4" />
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
                    title="Message"
                    className="hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded p-0.5 sm:p-1 transition h-5 w-5 sm:h-6 sm:w-6 lg:h-7 lg:w-7 xl:h-8 xl:w-8 text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300"
                    onClick={e => { e.stopPropagation(); setChatMember(member); }}
                  >
                    <MessageCircle className="w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-3.5 lg:h-3.5 xl:w-4 xl:h-4" />
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
                    title="Video Call"
                    className="hover:bg-purple-50 dark:hover:bg-purple-900/20 rounded p-0.5 sm:p-1 transition h-5 w-5 sm:h-6 sm:w-6 lg:h-7 lg:w-7 xl:h-8 xl:w-8 text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300"
                    onClick={e => { e.stopPropagation(); alert(`Starting video call with ${member.name}...`); }}
                  >
                    <Video className="w-2.5 h-2.5 sm:w-3 sm:h-3 lg:w-3.5 lg:h-3.5 xl:w-4 xl:h-4" />
                  </Button>
                </div>
              </li>
            ))}
